...
- [September 9 Atrium lunch menu](https://menuportal23.dining.rutgers.edu/FoodPronet/pickmenu.aspx?locationNum=13&dtdate=9%2F9%2F2026&activeMeal=Lunch&sName=) and the corresponding breakfast page, inspected for exact section names. Historical artwork research only; FoodProNet is no longer the menu source.
...
## Nutrislice migration — September 10, 2026

The current aliases in `lib/atrium-stations.ts` come from Rutgers’ `the-atrium` Nutrislice Breakfast/Lunch/Dinner sections. Brand-specific subcategories map to the existing brand zones; prepared salads, salad proteins and dressing join Soup & salad. HOT CEREAL, SPECIALTY SANDWICHES, SUSHI & POKE BOWLS and GRAB & GO WARMER remain under More stations because physical placement was not verified. No artwork, coordinates or supported hall changed. See [source evidence](../../tests/fixtures/nutrislice/atrium/README.md).
