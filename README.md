...
## Livingston and Busch station-map prototypes

`/hall/livingston` and `/hall/busch` open responsive 2D illustrated station guides. Select a numbered area to browse its menu items and add to the existing plate; use search across all stations or switch to the full list. Unknown/ambiguous station labels remain accessible under **More stations**. Meal counts and food panels use only the existing menu result, including the honest unavailable state. Neilson and Atrium retain their existing interface.
