...
test('maps, unavailable status, security headers and small-screen layout',async({page})=>{
  ...
  await page.goto('/hall/neilson');await expect(page.getByRole('heading',{name:/^(Breakfast|Lunch|Dinner) menu unavailable right now\.$/})).toBeVisible();await expect(page.getByRole('button',{name:/^Add /})).toHaveCount(0);
